...
class RegisterVC: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if pickerView == ramaPicker {Rama.text = ramas[row]; activarBoton()}
    ...
    func activarBoton(){
        ...
        let e = Edad.text! != edades[0] && Edad.text!.count != 0
        let f = Pais.text! != paises[0] && Pais.text!.count != 0
        let g = Genero.text! != generos[0] && Genero.text!.count != 0
